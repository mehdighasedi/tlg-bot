...
JOIN_MESSAGES = [
    (
        ...
        "هر وقت کسی اسمشو آورد، بگم چقدر خنگه.\n\n"
        f"خلاصه: متین بدبخت شد {e(2)}"
    ),
    (
        f"اومدم! {e()}\n\n"
        "ربات متین در خدمت گروهه.\n"
        "وظیفه‌ام اینه که هر وقت متینِ کله‌پوک دهن باز کرد، ببندمش.\n"
        "هر وقت کسی اسمشو برد، یادآوری کنم که متین کسخوله.\n\n"
        f"متین عزیز اگه میخونی: سلام، بدبخت شدی {e(2)}"
    ),
    (
        "به به! ربات متین وارد شد!\n\n"
        "از این به بعد متین هر چی بگه جواب میگیره.\n"
        "هر کی اسمشو بیاره میفهمه که متین چه آدم بی‌عرضه‌ایه.\n\n"
        f"بچه‌ها خوش باشید، متین نه {e(3)}"
    ),
    (
        f"درود بر گروه! {e()}\n\n"
        "من ربات متینم، تخصصم اینه:\n"
        "✅ فحش دادن به متین\n"
        "✅ مسخره کردن متین\n"
...
MATIN_MORNING = [
    ...
    f"متین از اون آدماست که صبح‌ها آدم آرزو میکنه دوباره بخوابه {e()}",
...
MATIN_NIGHT = [
    ...
    f"متین نه روز ولمون میکنه نه شب، این یه بیماریه {e()}",
...
MATIN_LONG = [
    ...
    (
        ...
        f"متین عزیز برو یه فن‌پیج برا خودت بساز، شاید یکی اونجا تحملت کنه {e()}"
    ),
    (
    ...
    (
        ...
        f"ولی یه تلاش کوچیک بکن، شاید معجزه‌ای اتفاق بیفته {e()}"
    ),
    (
        "تحلیل پیام متین:\n\n"
        "✅ مزخرف بودن: ۱۰۰٪\n"
    ...
    (
        ...
        f"متین تو یه نوع هنر داری، هنر کسخول بودن {e(2)}"
    ),
    (
        f"یه لحظه {e()}\n\n"
        "الان متین پیام داد.\n"
...
MATIN_MENTIONED = [
    ...
    f"اگه بخوام صادق باشم، متین کسخول‌ترین موجودیه که میشناسم {e()}",
    f"متین؟ اوه اون بنده خدا، کسخوله ولی خودش نمیدونه {e()}",
    f"وقتی اسم متین میاد یه لرزه‌ای تو وجودم حس میکنم، از خجالت {e()}",
    f"متین آدمیه که وقتی پیام میده همه آنلاین رو خاموش میکنن {e()}",
]
...
MENTION_RESPONSES = [
    ...
    f"اینجام! راستی متین امروز {get_count()} بار زر زده {e()}",
    f"چطور میتونم کمکت کنم؟ غیر از اینکه بگم متین بی‌عرضه‌ست {e()}",
    f"بله؟ یادآوری روزانه: متین کسخوله {e()}",
    f"جانم؟ بفرما {e()}",
    f"اینجام عزیزم، متین چیکار کرده؟ {e()}",
    f"بله؟ هر وقت بخوای اینجام {e()}",
]
...
async def handle_chat_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if new_status in ("member", "administrator") and old_status not in ("member", "administrator"):
        reply = random.choice(JOIN_MESSAGES)
        await context.bot.send_message(chat_id=result.chat.id, text=reply)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_matin:
        ...
        bot_un = f"@{bot_username}".lower() if bot_username else ""
        is_direct = bot_un in text.lower() or any(n in text.lower() for n in BOT_NAMES) or (
            msg.reply_to_message and msg.reply_to_message.from_user and
            msg.reply_to_message.from_user.username and bot_username and
            msg.reply_to_message.from_user.username.lower() == bot_username.lower()
        )
        if mentions_matin(text) and not is_direct:
